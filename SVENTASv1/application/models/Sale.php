...
class Sale extends CI_Model
{
	...
	function save ($items,$customer_id,$employee_id,$comment,$payments,$sale_id=false)
	{
		...
		// múltiples pagos
		// Construir cadena tipos de pago
		$payment_types='';
		...
		foreach($items as $line=>$item)
		{
			...
			//actualizacion de cantidad en stock
			$item_data = array('quantity'=>$cur_item_info->quantity - $item['quantity']);
			$this->Item->save($item_data,$item['item_id']);
			
			// Seguimiento de inventario 
			// Detalles recuento de inventario
			$qty_buy = -$item['quantity'];
			$sale_remarks ='FRA. '.$sale_id;
			$inv_data = array
			(
	...
	//Creamos una tabla temporal que nos permite hacer consultas fáciles de informes de ventas
	public function create_sales_items_temp_table()
	{
		...
		".$this->db->dbprefix('sales_items').".line as line, ".$this->db->dbprefix('sales_items').".description as description,
		...
		//Actualizar porcentajes nulos de impuestos debe ser 0 en lugar de null
		$this->db->where('item_tax_percent IS NULL');
		$this->db->update('sales_items_temp', array('item_tax_percent' => 0));

		//Actualización tributaria nula sea 0 en lugar de null
		$this->db->where('tax IS NULL');
		$this->db->update('sales_items_temp', array('tax' => 0));

		//Actualización de subtotales nulos para que sea igual al total ya que estos no tienen impuesto
