...
{	
	...
	//Pantalla con una lista del Informe inicial
	function index()
	{
	...
	//Da información para informes que requieren solamente un intervalo de fechas y una exportación a excel . (Ver routes.php para ver que todas las rutas informes resumidos aquí)
	...
	//información para informes que requieren solamente un intervalo de fechas . (Ver routes.php para ver que todos los informes de resumen gráfico de ruta aquí)
	function date_input()
	{
		$data = $this->_get_common_report_data();
	...
	//Resumen gráfico informe de ventas
	...
	//Resumen del reporte Gráfico de clientes
	...
	//Resumen del reporte Gráfico de descuentos
	...
	function specific_customer($start_date, $end_date, $customer_id, $sale_type, $export_excel=0)
	{
		...
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], 'FRA. '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['employee_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']), $row['payment_type'], $row['comment']);
	...
	function specific_employee($start_date, $end_date, $employee_id, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], 'FRA. '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['customer_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']), $row['payment_type'], $row['comment']);
	...
	function detailed_sales($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], 'FRA. '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['employee_name'], $row['customer_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']), $row['payment_type'], $row['comment']);
